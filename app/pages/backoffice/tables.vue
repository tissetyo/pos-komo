...
const deleteTable = async (table: any) => {
  ...
  await client.from('tables').delete().eq('id', table.id)
  await fetchTables()
}

const toggleActive = async (table: any) => {
  await client.from('tables').update({ is_active: !table.is_active }).eq('id', table.id)
  await fetchTables()
}
...
const showQr = async (table: any) => {
  ...
  qrDataUrl.value = await QRCode.toDataURL(url, {
    width: 400,
    margin: 2,
    color: { dark: '#162456', light: '#ffffff' }
  })
}
        ...
        <div class="flex items-start justify-between mb-4">
          <div class="flex items-center gap-3">
            <div :class="['w-12 h-12 rounded-xl flex items-center justify-center text-lg font-bold',
              table.is_active ? 'bg-[#162456] text-white' : 'bg-gray-200 text-gray-400']">
              {{ table.table_number }}
            </div>
            <div>
              <h4 class="font-semibold text-gray-900">Table {{ table.table_number }}</h4>
